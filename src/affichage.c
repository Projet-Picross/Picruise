...
void affiche_jeu(SDL_Window * window, SDL_Renderer * renderer, SDL_Rect * r, int * grille, int * mat_cond_l, int * mat_cond_c, int format, int nb_max_cond_lignes, int nb_max_cond_colonnes){
    ...
    //Affichage de la fenêtre du timer
    ...
    //Initialisation de la police d'écriture pour le timer
    ...
    //Coordonnées et dimensions de la zone d'affichage
    SDL_Rect dstrect = { 0, 0, texW, texH };

    //Calcul du nombre maximal de conditions
    ...
    for(i = 0; i < format; i++){
      for(j = nb_max_cond_colonnes; j >= 0; j--){
        if(mat_cond_c[i*format + j]){
          ...
          //Alterne entre case à fond gris et fond marron pour plus de clarté
          if(j%2){
    ...
    for(i=0;i<format;i++){
      for(j=0;j<format;j++){
        //Initialisation des Coordonnées et dimensions de chaque case de la grille
        ...
        //Affichage de la case selon sa valeur
